...
class Prompter:
    ...
    def prompt_for_extended_data(self):
        self.is_extended_data = self.prompt(
    ...
    def prompt_for_dataset_count(self):
        self.datasets_count = self.prompt(
    ...
    def prompt_for_results_amount(self):
        self.results_amount = self.prompt(
...
class DataLoader:
    ...
    def load_data(self, is_extended_data, datasets_count, dataset_path):
        if is_extended_data:
            ...
            return self.load_default_data(datasets_count, dataset_path)

    def load_default_data(self, datasets_count, dataset_path):
            ...
            if file_path.exists():
                try:
                    with open(file_path, 'r', encoding='utf-8') as file:
                        data = json.load(file)
                        for item in data:
                            if (item['master_metadata_track_name'] is not None and
                                item['master_metadata_album_artist_name'] is not None and
                                    item['master_metadata_album_album_name'] is not None):
                                yield (item['master_metadata_track_name'],
...
if __name__ == "__main__":
    ...
    logging.info(f"Starting Spotify Data Analyser...")
    spotify_data_analyser = SpotifyDataAnalyser().main()
    logging.info(f"Spotify Data Analyser analysed and data displayed.")
